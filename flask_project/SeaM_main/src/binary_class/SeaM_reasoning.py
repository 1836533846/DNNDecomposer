...
class CIFAR10Inference:
    ...
    def predict(self,image_path):
        ...
        transform = transforms.Compose([
            ...
            transforms.ToTensor(),
            # CIFAR-10数据集的RGB通道均值&标准差
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
        ])

        img = Image.open(image_path).convert("RGB")
        transformed_image = transform(img)
        input_data = transformed_image.unsqueeze(0).to(self.device)  # unsqueeze(0)为了增加batch维度
        ...
        # print(f"Predicted class label: {label_num}")
...
mask_path = "SeaM_main/data/binary_classification/vgg16_cifar10/tc_3/lr_head_mask_0.1_0.01_alpha_1.0.pth"
